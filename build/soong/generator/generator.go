...
func init() {
	android.RegisterModuleType("pixelbuilds_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = pixelbuildsExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = pixelbuildsExpandVariables(ctx, dep_file)
	...
	cmd := pixelbuildsExpandVariables(ctx, String(g.properties.Cmd))
