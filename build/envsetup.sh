...
- githubremote:    Add git remote for PixelBuilds Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the PixelBuilds model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch pixelbuilds_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/pixelbuilds-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.pixelbuilds.device | grep -q "$PIXELBUILDS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $PIXELBUILDS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/PixelBuildsROM/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.pixelbuilds.device | grep -q "$PIXELBUILDS_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.pixelbuilds.device | grep -q "$PIXELBUILDS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/pixelbuilds/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $PIXELBUILDS_FIXUP_COMMON_OUT ]; then
